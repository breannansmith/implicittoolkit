...
void MarchingCubes::run( real iso )
{
  ...
  for( _k = 0 ; _k < _size_z-1 ; _k++ ) for( _j = 0 ; _j < _size_y-1 ; _j++ ) for( _i = 0 ; _i < _size_x-1 ; _i++ )
  {
    ...
    for( int p = 0 ; p < 8 ; ++p )
    {
      ...
      if( fabs( _cube[p] ) < double(FLT_EPSILON) ) _cube[p] = double(FLT_EPSILON) ;
...
void MarchingCubes::init_temps()
{
  if( !_ext_data )
  {
    _data = new real[static_cast<unsigned long>(_size_x * _size_y * _size_z)];
  }
  _x_verts = new int[static_cast<unsigned long>(_size_x * _size_y * _size_z)];
  _y_verts = new int[static_cast<unsigned long>(_size_x * _size_y * _size_z)];
  _z_verts = new int[static_cast<unsigned long>(_size_x * _size_y * _size_z)];

  memset( _x_verts, -1, static_cast<unsigned long>(_size_x * _size_y * _size_z) * sizeof(int) ) ;
  memset( _y_verts, -1, static_cast<unsigned long>(_size_x * _size_y * _size_z) * sizeof(int) ) ;
  memset( _z_verts, -1, static_cast<unsigned long>(_size_x * _size_y * _size_z) * sizeof(int) ) ;
}
...
void MarchingCubes::init_all()
{
  ...
  _vertices  = new Vertex[static_cast<unsigned long>(_Nverts)];
  _triangles = new Triangle[static_cast<unsigned long>(_Ntrigs)];
}
...
//-----------------------------------------------------------------------------
{
  ...
  for( _i = 0 ; _i < _size_x ; _i++ )
  {
    ...
    if( fabs( _cube[0] ) < double(FLT_EPSILON) ) _cube[0] = double(FLT_EPSILON) ;
    if( fabs( _cube[1] ) < double(FLT_EPSILON) ) _cube[1] = double(FLT_EPSILON) ;
    if( fabs( _cube[3] ) < double(FLT_EPSILON) ) _cube[3] = double(FLT_EPSILON) ;
    if( fabs( _cube[4] ) < double(FLT_EPSILON) ) _cube[4] = double(FLT_EPSILON) ;
...
//-----------------------------------------------------------------------------
{
  ...
  if( fabs( A*C - B*D ) < double(FLT_EPSILON) )
    return face >= 0 ;
...
//-----------------------------------------------------------------------------
{
  ...
  case  3 : return s>0 ;
  case  4 : return s>0 ;
  case  5 : if( At * Ct - Bt * Dt <  double(FLT_EPSILON) ) return s>0 ; break ;
  case  6 : return s>0 ;
  case  7 : return s<0 ;
  case  8 : return s>0 ;
  case  9 : return s>0 ;
  case 10 : if( At * Ct - Bt * Dt >= double(FLT_EPSILON) ) return s>0 ; break ;
  case 11 : return s<0 ;
  case 12 : return s>0 ;
  case 13 : return s<0 ;
  case 14 : return s<0 ;
  ...
  for( int t = 0 ; t < 3*n ; t++ )
  {
    ...
    if( t%3 == 2 )
    {
      if( _ntrigs >= _Ntrigs )
      {
        ...
        _triangles = new Triangle[ static_cast<unsigned long>(2*_Ntrigs) ] ;
        memcpy( _triangles, temp, static_cast<unsigned long>(_Ntrigs)*sizeof(Triangle) ) ;
        delete[] temp ;
...
void MarchingCubes::test_vertex_addition()
{
  if( _nverts >= _Nverts )
  {
    ...
    _vertices = new Vertex[ static_cast<unsigned long>(_Nverts*2) ] ;
    memcpy( _vertices, temp, static_cast<unsigned long>(_Nverts)*sizeof(Vertex) ) ;
    delete[] temp ;
