...
// Accessors
  ...
  {
    if( i < 0  || i >= _nverts ) return nullptr;
    return _vertices + i;
  }
  /** accesses a specific triangle of the generated mesh */
  inline Triangle* trig( const int i ) const
  {
    if( i < 0  || i >= _ntrigs ) return nullptr;
    return _triangles + i;
  }
  ...
  inline void set_ext_data( real* data )
  {
    if( !_ext_data )
    {
      //assert( _data != nullptr ); Why does this fail? Strange.
      delete[] _data;  
    }
    _ext_data = data != nullptr;
    if( _ext_data )
    {
      _data = data;
    }
